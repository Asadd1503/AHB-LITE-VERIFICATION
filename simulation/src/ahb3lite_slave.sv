...
(
  //input                       if_ahb_dut.HRESETn,
  input                       HCLK,

  //input                       if_ahb_dut.HSEL,
  //input      [HADDR_SIZE-1:0] if_ahb_dut.HADDR,
  //input      [HDATA_SIZE-1:0] if_ahb_dut.HWDATA,
  //output reg [HDATA_SIZE-1:0] if_ahb_dut.HRDATA,
  //input                       if_ahb_dut.HWRITE,
  //input      [           2:0] if_ahb_dut.HSIZE,
  //input      [           2:0] if_ahb_dut.HBURST,
  //input      [           3:0] HPROT,
  //input      [           1:0] if_ahb_dut.HTRANS,
  output reg                  HREADYOUT,
  input                       HREADY,
  //output                      HRESP
  if_ahb.ahb_dut if_ahb_dut
);
...
timeprecision 1ns;
  ...
  function automatic logic [BE_SIZE-1:0] gen_be;
    ...
    //get number of active lanes for a 1024bit databus (max width) for this if_ahb_dut.HSIZE
    case (hsize)
  ...
  assign ahb_noseq = !if_ahb_dut.HSEL || (if_ahb_dut.HTRANS == HTRANS_IDLE) || (if_ahb_dut.HTRANS == HTRANS_NONSEQ);
  assign ahb_write = if_ahb_dut.HSEL &  if_ahb_dut.HWRITE & (if_ahb_dut.HTRANS != HTRANS_BUSY) & (if_ahb_dut.HTRANS != HTRANS_IDLE);
  assign ahb_read  = if_ahb_dut.HSEL & ~if_ahb_dut.HWRITE & (if_ahb_dut.HTRANS != HTRANS_BUSY) & (if_ahb_dut.HTRANS != HTRANS_IDLE);

  always @(posedge HCLK, negedge if_ahb_dut.HRESETn)
    if      (!if_ahb_dut.HRESETn) was_ahb_noseq <= 1'b1;
  ...
  //decode Byte-Enables
  always @(posedge HCLK)
    if (HREADY) be <= gen_be(if_ahb_dut.HSIZE,if_ahb_dut.HADDR);

  //read address
  assign nxt_adr = gen_nxt_adr(if_ahb_dut.HADDR, if_ahb_dut.HSIZE, if_ahb_dut.HBURST);
  assign raddr = if_ahb_dut.HADDR[MEM_ABITS_LSB +: MEM_ABITS]; 
  ...
  mem #(
    ...
    .DBITS      ( HDATA_SIZE ))
  mem_inst (
    .rst_ni  ( if_ahb_dut.HRESETn ),
    .clk_i   ( HCLK    ),

    .addr_i  ( waddr   ),
    .we_i    ( we      ),
    .be_i    ( be      ),
    .din_i   ( if_ahb_dut.HWDATA  ),
    .dout_o  ( dout    )
  );
  ...
  always @(posedge HCLK,negedge if_ahb_dut.HRESETn)
    if      (!if_ahb_dut.HRESETn                          ) HREADYOUT <= 1'b1;
    else if ( ahb_noseq && ahb_read & HREADYOUT) HREADYOUT <= 1'b0;
    else                                         HREADYOUT <= 1'b1;

  always @(posedge HCLK)
     if_ahb_dut.HRDATA <= contention ? dout_local : dout;
